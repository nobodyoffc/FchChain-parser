	...
	public int startParse(ElasticsearchClient esClient) throws Exception {
		...
		while(true) {
			...
			case LINK_MAIN:
				...
				PreparerTest.BestHash = blockMark.getId();
...
//			}
	...
	private CheckResult checkBlock(FileInputStream fis) throws Exception {
		...
		System.out.println("Orphan block. ID:" +blockMark.getId()+"preID"+blockMark.getPreId()
	...
	private boolean isLinkToMain(BlockMark blockMark) {
		// TODO Auto-generated method stub
		if(blockMark.getPreId().equals(PreparerTest.BestHash)){
	...
	private void reorganize(ElasticsearchClient esClient, BlockMark blockMark) throws Exception {
		...
		if(winList == null || winList.isEmpty()) return;
			...
			System.out.println("Reorganized. Fork: "+PreparerTest.forkList.size()+"BlockId before fork: "+blockMarkBeforeFork.getId()+" Height: "+blockMarkBeforeFork.getHeight());
	}
		...
		while(!found) {	
			...
			while(iter.hasNext()) {
				...
				if(blockMark.getPreId().equals(PreparerTest.BestHash)) {
					...
					ReadyBlock rawBlock = parseBlock(blockBytes,blockMark);
					//ReadyBlock readyBlock = new BlockMaker().makeReadyBlock(esClient, rawBlock);
					//writeIntoEs(esClient, readyBlock,opReFile);
					PreparerTest.mainList.add(blockMark);
					if (PreparerTest.mainList.size() > EsTools.READ_MAX) {
						PreparerTest.mainList.remove(0);
					}
					...
					if(bestBlockMark.getId()!= PreparerTest.BestHash) {
						System.out.println("BestHash "+PreparerTest.BestHash+" is not the same as mainList:"+bestBlockMark.getId());
						throw new Exception("BestHash "+PreparerTest.BestHash+" is not the same as mainList:"+bestBlockMark.getId());
					}
	...
	private void deleteMarkFromEs(ElasticsearchClient esClient, BlockMark blockMark) throws ElasticsearchException, IOException {
		// TODO Auto-generated method stub
		//esClient.delete(d->d.index(Indices.BlockMarkIndex).id(blockMark.getId()));
	}
